...
export class TaskEditComponent extends MagpieEditComponent {
  @Input()
  section_data: any;
  columns: any;
  section_alias: any;
  ...
  constructor(public route: ActivatedRoute, public router: Router, public fb: FormBuilder, public http: HttpClient, public section_service: SectionService, public ref: ChangeDetectorRef, public spinnerService: Ng4LoadingSpinnerService, public webSocketService: WebsocketService,public subsectionService: SubsectionService) {
    ...
    this.parent_uri = this.router.url.split('edit')[0];
  ...
  update = () => {
    ...
    this.route.params.subscribe(params => {
      ...
      this.subsectionService.update(this.section_data, this.current_section).subscribe(res => {
  ...
  setEditColumnsData = (params_id, section, config_columns, res1, th_files, th_tags, res) => {
    this.subsectionService.edit(params_id, section).subscribe(res3 => {
              ...
              config_columns.forEach(function (rowItem, key) {
                if ((rowItem.type == 'tags' || rowItem.type == 'selectbox' || rowItem.type == 'checkbox' || rowItem.type == 'radio' || rowItem.type == 'autocomplete') && rowItem.source_type == 'dynamic') {
                  if (th_router.url.split('/')[2] == 'users' || th_router.url.split('/')[2] == 'roles' || th_router.url.split('/')[2] == 'menus' || th_router.url.split('/')[2] == 'sections') {
                    th_service.customRoute(th.current_section, rowItem.source_from).subscribe(res2 => {
                      if (rowItem.type == 'tags') {
                        ...
                        th.setEditColumnsData(params['id'], th.current_section, config_columns, res1, th_files, th_tags, res);
                      }
                      else
                        rowItem.source = res2;
                    });
                  } else {
                    th_service.adminCustomRoute(th.current_section, rowItem.source_from).subscribe(res2 => {
                      if (rowItem.type == 'tags') {
                        var data_tags = [];
                        for (var k in res2) {
                          data_tags.push({ 'id': res2[k].value, 'text': res2[k].label });
                        ...
                        th_tags.push(rowItem.field);
                        rowItem.source = data_tags;
                        th.setEditColumnsData(params['id'], this.current_section, config_columns, res1, th_files, th_tags, res);
                      }
                      else
                        rowItem.source = res2;
                    });
