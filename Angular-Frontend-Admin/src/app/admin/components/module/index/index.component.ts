...
import { IndexpmsService } from '../../../services/indexpms.service';
...
export class ModuleIndexComponent extends MagpieIndexComponent {
  @Input()
  section_data: any;
  section_config: any;
  columns: any = "";
  ...
  export = (type) => {
    this.section_service.getCurrentRolePermissionMenus('roles', localStorage.getItem("userDetails['roles_id']")).subscribe(res1 => {
      ...
      if (menus_actions['Export']) {
        this.section_service.sectionConfig('/admin/module').subscribe(res => {
          const columns_array = JSON.parse(res[0].section_config).column;
          this.route.params.subscribe(params => {
            this.section_service.export(this.search_word, this.searchable_fields, this.order_fieldBy, this.order_field, this.column_relation, '/admin/module', columns_array).subscribe(result => {
              if (type == 'csv') {
                var options = {
                  useBom: true,
                  noDownload: false,
    ...
    this.section_service.getCurrentRolePermissionMenus('roles', localStorage.getItem("userDetails['roles_id']")).subscribe(res1 => {
      ...
      if (menus_actions['Index']) {
        this.section_service.sectionConfig('/admin/module').subscribe(res => {
          ...
          this.subsectionService.get(this.current_page, this.per_page, this.order_field, this.order_fieldBy, 'module',{'project':this.project_id}).subscribe(result => {
            this.section_data = result['data'];
             this.section_data.map((item) => {
              this.indexpmsservice.getSubCount('task',item.module_id,'module').subscribe(result => {
                item.no_task=result>0?result:0;
              });
