...
export class ProjectIndexComponent extends MagpieIndexComponent {
  @Input()
  section_data: any;
  section_config: any;
  columns: any = "";
  ...
  onPerpageChange = (perPage) => {
    ...
    this.route.params.subscribe(params => {
      this.section_service.search(this.search_word, this.searchable_fields, this.order_fieldBy, this.order_field, this.column_relation, this.current_page, this.per_page, this.router.url).subscribe(result => {
        this.section_data = result['data'];
        this.pages = result['pages'];
        this.current = result['current'];
        this.paginate_total = result['tot'];
  ...
  onSearchChange = (value) => {
    ...
    this.spinnerService.show();
    this.route.params.subscribe(params => {
      this.section_service.search(value, this.searchable_fields, this.order_fieldBy, this.order_field, this.column_relation, this.current_page, this.per_page, this.router.url).subscribe(result => {
        this.section_data = result['data'];
        this.pages = result['pages'];
        this.current = result['current'];
        this.paginate_total = result['tot'];
          ...
          this.section_service.get(this.current_page, this.per_page, this.order_field, this.order_fieldBy, this.router.url).subscribe(result => {
            this.section_data = result['data'];
             this.section_data.map((item) => {
              this.indexpmsservice.getSubCount('module',item.project_id,'project').subscribe(result => {
                item.no_module=result>0?result:0;
              });
              this.indexpmsservice.getSubCount('timesheet',item.project_id,'project').subscribe(result => {
                item.no_tsheet=result>0?result:0;
              });
            });
            this.pages = result['pages'];
            this.current = result['current'];
