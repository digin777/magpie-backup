  ...
  try {
    ...
    if (token) {
      customIsPermission(token, data = {}).then((is_perm) => {
        if (is_perm) {
          Section.find({ parent_id: 0, is_deleted: { $ne: 1 }, }, 'name menus_id', function (err, result) {
            if (err) {
              console.log(err);
            }
            else {
...
customAdminRoutes.route('/getRoles').post(function (req, res) {
  try {
    ...
    if (req.body.role_id != 1)
      where = { "roles_id": { $ne: 1 }, "created_user_id": decode.users_id, is_deleted: { $ne: 1 }, status: "active" };
    else
      where = { status: "active", is_deleted: { $ne: 1 }, };
    if (token) {
      customIsPermission(token, data = {}).then((is_perm) => {
        if (is_perm) {
